...
def train(logger, train_loader, model, criterion, optimizer, epoch, num_iter_for_update):
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        if num_iter_for_update == 1 :
            losses.update(loss.data, inputs.size(0))    # problem: when num_iter_for_update > 1, loss still be calculated for each batch and last remaining batch loss is also added
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='logs_invariant/',
                        help='Directory name to save the checkpoints')    
    ...
    ####################################################################################################
    os.makedirs(local_dir+args.save_dir+args.dataset+'/'+args.net_type+'/batch_'+str(args.batch_size), exist_ok=True)
    ...
    copy_dir = '{}{}/{}/batch_{}/\
invariant_alpha_sq_{}_WD_{}_lr{}_eps_{}_zero_init_{}_warm_iter_{}_filter_bn_bias_{}_momentum{}_nester_{}_epoch{}_amp_{}_seed{}_'.format(
        args.save_dir, args.dataset, args.net_type, args.batch_size, \
        args.alpha_sqaure, args.weight_decay, args.lr, args.eps, args.zero_init_residual, args.warm_up_epoch, args.filter_bn_bias, \
