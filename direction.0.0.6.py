'''
******  Experiment Whether the Gradient is Constant  ******
direction.0.0.4.py에서 복사 - 리부탈 reviewer 2 gradient constant 질문 확인 실험
'''
...
def train(logger, train_loader, model, criterion, optimizer, epoch, update_freq, lr_schedule, conv_parameters):
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        # for gradient test 
        if epoch in args.grad_test_epoch and i==0:
            fixed_input = inputs # shallow copy
            fixed_target = targets
        ...
        if update_freq == 1 :       # 이거 없어도 되는거 같은데 왜 했지?????
        ...
        if args.amp:    # for backward compatibility 
            ...
            if (i+1) % update_freq == 0 :
                if args.max_grad_norm is not None:
                    nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)    
                optimizer.step()
        if (i+1) % update_freq == 0 and i+1 != len(train_loader): # except for last iteration because we should log gradient norm
            for param in model.parameters():
                param.grad = None

        # Here, we don't consider update_freq for simplicity. Do not use very large batch 
        if epoch in args.grad_test_epoch:
            model.eval()

            with autocast():
                outputs = model(fixed_input)
                loss = criterion(outputs, fixed_target)
                scaler.scale(loss).backward()
            if i==0:
                weight_diff_logging, gradient_diff_logging = [],[]
                base_weight, base_gradient = vectorize(model, conv_parameters)
                current_weight = base_weight
                current_gradient = base_gradient
            else: 
                current_weight, current_gradient = vectorize(model, conv_parameters)
            weight_diff = (base_weight-current_weight).norm(2) / base_weight.norm(2)
            gradient_diff = (base_gradient-current_gradient).norm(2) / current_gradient.norm(2)
            weight_diff_logging.append(weight_diff.item())
            gradient_diff_logging.append(gradient_diff.item())
            logger.info('Difference\t weight: {}\t gradient: {}'.format(weight_diff, gradient_diff))
    ...
    if epoch in args.grad_test_epoch:
        ...
        fig, ax = plt.subplots()
        image, = ax.plot(np.arange(len(train_loader)), weight_diff_logging, linewidth=3, alpha=0.9)
        ax.set_xlabel('iteration', fontsize=18)
        ax.set_ylabel('weight difference', fontsize=18)    
        plt.gcf().subplots_adjust(bottom=0.15)
        plt.gcf().subplots_adjust(left=0.15)
        fig.savefig('{}/weight_difference_{}.pdf'.format(args.save_dir,epoch), dpi=300)
        plt.cla()
        plt.close(fig)

        fig, ax = plt.subplots()
        image, = ax.plot(np.arange(len(train_loader)), gradient_diff_logging, linewidth=3, alpha=0.9)
        ax.set_xlabel('iteration', fontsize=18)
        ax.set_ylabel('gradient difference', fontsize=18)    
        plt.gcf().subplots_adjust(bottom=0.15)
        plt.gcf().subplots_adjust(left=0.15)
        fig.savefig('{}/gradient_difference_{}.pdf'.format(args.save_dir,epoch), dpi=300)
        plt.cla()
        plt.close(fig)
...
def vectorize(model, conv_parameters):
    model.eval()
    with torch.no_grad():
        wt_l2_norm, grad_l2_norm, conv_l2_norm, conv_grad_l2_norm = 0,0,0,0
        wt_weight, wt_conv_weight = [],[]
        wt_gradient, wt_conv_gradient = [],[]
        for name, param in model.named_parameters():
            if not 'linear' in name and not 'classifier' in name:   # except for fully connected layer
                norm_square = param.data.norm(2).item()**2
                grad_norm_square = param.grad.data.norm(2).item()**2
                weight_vector = param.view(-1).data
                gradient_vector = param.grad.view(-1).data
                # all weight 
                wt_l2_norm = wt_l2_norm + norm_square
                grad_l2_norm = grad_l2_norm + grad_norm_square
                wt_weight.append(weight_vector)
                wt_gradient.append(gradient_vector)
                # conv weight
                # if any(conv_parameter in name for conv_parameter in conv_parameters):
                #     conv_l2_norm = conv_l2_norm + norm_square
                #     conv_grad_l2_norm = conv_grad_l2_norm + grad_norm_square
                #     wt_conv_weight.append(weight_vector)
                #     wt_conv_gradient.append(gradient_vector)
        wt_weight = torch.cat(wt_weight,0)
        wt_gradient = torch.cat(wt_gradient,0)
        # wt_conv_weight = torch.cat(wt_conv_weight,0)
        # wt_conv_gradient = torch.cat(wt_conv_gradient,0)
        wt_l2_norm = wt_l2_norm**(1./2)
        grad_l2_norm = grad_l2_norm**(1./2)
        conv_l2_norm = conv_l2_norm**(1./2)
        conv_grad_l2_norm = conv_grad_l2_norm**(1./2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--grad_test_epoch', type=int, nargs='+', default=[99,199,299], 
                        help='Learning Rate Decay Steps')
    ...
    os.makedirs(local_dir+args.save_dir+args.dataset+'/'+args.net_type+'_width_'+str(args.width)+'_'+args.optim+'/num_data_'+
    ...
    copy_dir = '{}{}/{}_width_{}_{}/num_data_{}/batch_{}/\
{}_WD_{}_lr{}_warmup_{}_filter_bn_bias_{}_moment_{}_nester_{}_epoch{}_amp_{}_seed{}_'.format(
        args.save_dir, args.dataset, args.net_type, args.width, args.optim, args.num_sample, args.batch_size, \
