...
# GCP
docker run --gpus all --shm-size=40g --ulimit memlock=-1 --ulimit stack=67108864 -it --rm --pid=host -v \
	-p 8000-8100:8000-8100 -v /etc/localtime:/etc/localtime:ro -v /etc/passwd:/etc/passwd:ro -v /etc/group:/etc/group:ro \
	-v /shared_storage:/home/user/ --name juseung st24hour/pytorch:1.10.0_angular_update \
	sh train7.sh

# docker run --gpus all --shm-size=40g --ulimit memlock=-1 --ulimit stack=67108864 -it --rm --pid=host -v -p 8000-8100:8000-8100 -v /etc/localtime:/etc/localtime:ro -v /etc/passwd:/etc/passwd:ro -v /etc/group:/etc/group:ro -v /shared_storage:/home/user/ --name juseung st24hour/pytorch:1.10.0_angular_update
###############################################################################################
